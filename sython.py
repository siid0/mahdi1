...
Gg مثال على ذالك 
/somy eeobot 300 يجمع لبوت المليار
    ...
    while True:
        ...
        try:
            ...
            if sender.id == ownerhson_id:
                ...
                await sython1.send_message(pot, '**جاري بدأ عملية التجميع بواسطة سورس السيد **')
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        joinr = await sython1(JoinChannelRequest('FFF22'))

        joint = await sython1(JoinChannelRequest('q2qqqq'))

        joiny = await sython1(JoinChannelRequest('zzzzzz'))

        joini = await sython1(JoinChannelRequest('S_8_D'))

        joino = await sython1(JoinChannelRequest('lI7777Il'))

        joinp = await sython1(JoinChannelRequest('KTTTT'))

        joina = await sython1(JoinChannelRequest('ujijs'))
